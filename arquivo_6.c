...
int main(){
    ...
    int i, j, diagonal = 0;
    //OS ELEMENTOS DA DIAGONAL PRINCIPAL, SÃO AQUELES NOS QUAIS, OS ÍNDICES DAS LINHAS SÃO IGUAIS AOS DAS COLUNAS!
    //LAÇO FOR ANINHADO!
    for(i = 0; i < DIMENSAO; i++){
        for(j = 0; j < DIMENSAO; j++){
            ...
            if(scanf("%d", &matriz[i][j]) != 1 || (matriz[i][j] > 10 || matriz[i][j] < 10)){
